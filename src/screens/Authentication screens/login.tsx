...
import { LoginSetup , LoginWithFacebook, LoginWithGoogle} from '../../config/firebase'
...
function UploadPost() {
  ...
  return (
    <div className='bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 h-screen'>
      ...
      <div className='w-[400px] bg-white shadow-2xl rounded-2xl px-3 py-5'>
           <h2 className='text-[30px] font-semibold text-center dark:text-[black]'>Scroll Link</h2> 
           ...
           <div className='flex gap-2'>
            <button onClick={LoginWithFacebook} className='bg-[#2260E6] text-white p-2 rounded-xl text-[18px] flex justify-center items-center my-2 gap-2 w-full'><FaFacebook className='text-[22px]'/> facebook</button>
            <button onClick={LoginWithGoogle} className='border-[1px] border-black p-2 rounded-xl text-[18px] flex justify-center items-center my-2 gap-2 w-full'><FcGoogle  className='text-[22px]'/> Google</button>
           </div>
      </div>
        </div>
    </div>
